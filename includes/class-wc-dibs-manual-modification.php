...
class WC_Dibs_Manual_Modification {
	...
	public function dibs_transaction_metabox( $post_type ) {
		...
		add_meta_box(
			'wc_dibs_transaction_metabox',
			__( 'DIBS Order transaction details', 'woocommerce-dibs' ),
			array( $this, 'render_transaction_meta_box_content' ),
			'shop_order',
			'advanced',
			'high'
		);
	...
	public function render_transaction_meta_box_content( $post ) {
		...
		<div class="woocommerce_dibs_transaction_wrapper">
			<table cellpadding="0" cellspacing="0" class="woocommerce_order_items dibs_transaction_table">
				<thead>
					<tr>
						<th class="dibs-transaction"><?php _e( 'DIBS Transaction number', 'woocommerce' ); ?></th>
						<th class="dibs-ticket"><?php _e( 'DIBS Ticket number (for subscription payments)', 'woocommerce' ); ?></th>
					</tr>
				</thead>
				<tbody>
					<tr>
						<td class="dibs-transaction"><input type="text" name="woocommerce_dibs_transaction" id="woocommerce_dibs_transaction" value="<?php echo esc_attr( get_post_meta( $post->ID, '_dibs_transaction_no', true ) );?>" /></td>
						<td class="dibs-ticket"><input type="text" name="woocommerce_dibs_ticket" id="woocommerce_dibs_ticket" value="<?php echo esc_attr( get_post_meta( $post->ID, '_dibs_ticket', true ) );?>" /></td>
